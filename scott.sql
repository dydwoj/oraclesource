...
VALUES(7777, '이동욱', 'CREARK', 1111, SYSDATE, 2000, 500, 10);
...
-- EMP_TEMP 에서 SAL 이 3000 이상인 사람 삭제
...
UPDATE EXAM_EMP e
SET DEPTNO = 70
WHERE e.SAL > (SELECT AVG(e.SAL) FROM EXAM_EMP e WHERE e.DEPTNO = 50);

-- EXAM_EMP 에서 입사일이 가장 빠른 60번 부서 사원보다 늦게 입사한 사원의
-- 급여를 10% 인상하고 80번 부서로 옮기는 SQL 구문 작성
UPDATE EXAM_EMP e
SET SAL = (e.SAL * 1.1), DEPTNO = 80
WHERE e.HIREDATE > (SELECT MIN(e.HIREDATE) 
FROM EXAM_EMP e
WHERE e.DEPTNO = 60);
...
WHERE e.EMPNO IN (SELECT e.EMPNO FROM EXAM_EMP e JOIN EXAM_SALGRADE s ON e.SAL BETWEEN s.LOSAL AND s.HISAL
WHERE s.GRADE = 5);

-- 강사님꺼
DELETE FROM EXAM_EMP e
WHERE e.EMPNO IN (
SELECT e.EMPNO FROM EXAM_EMP e JOIN EXAM_SALGRADE s ON e.SAL BETWEEN s.LOSAL AND s.HISAL AND s.GRADE = 5);

-- 트랜잭션
-- COMMIT / ROLLBACK
INSERT INTO DEPT_TEMP VALUES(30, 'DATABASE', 'SEOUL');
UPDATE DEPT_TEMP SET LOC = 'BUSAN' WHERE DEPTNO = 30;
DELETE FROM DEPT_TEMP WHERE DNAME = 'DATABASE';
COMMIT;
...
-- 영어 : 14문자 / 한글(3바이트 사용) : 4문자
-- NUMBER(7, 2) : 전체자리수 7 (소수점 2자리 포함) => 정수는 5자리가 최대
...
-- 기존 테이블 구조와 데이터를 복사해서 새로운 테이블 생성
CREATE TABLE EXAM_EMP AS SELECT * FROM EMP;
...
VALUES ('hong222', '성춘향', '서울시 종로구', 'hong123@naver.com', 24);
...
-- MEMBER 테이블에 NO 컬럼 추가
ALTER TABLE MEMBER MODIFY NO NUMBER(20);
...
VALUES (MEMBER_SEQ.NEXTVAL, 'hong128', '홍길동', '서울시 종로구', 'hong123@naver.com', 24);
...
--	=> 시퀀스 SEQ_DEPT_SEQUENCE.NEXTVAL exceeds MAXVALUE은 사례로 될 수 없습니다
...
-- TBL_NOTNULL 의 TEL 컬럼을 NOT NULL 로 변경
...
INSERT INTO DEPT_FK VALUES(20, 'DATABASE', 'SEOUL');
...
-- ORA-02290: 체크 제약조건(SCOTT.SYS_C008393)이 위배되었습니다
INSERT INTO TBL_CHECK
VALUES('TEST_ID', '102', '010-1234-5678');
