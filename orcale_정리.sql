...
<< 제약조건 >> : ★★★★★ 아주중요 ★★★★★
	...
	5) CHECK : 데이터의 형태와 범위를 지정
		
		
	6) DEFAULT : 기본값 설정
		=> 지정해서 들어오는게 아닌 정해진 걸로 자동으로 들어가게 하는것
		

<< 사용자 관리 >>
	=> 오라클은 테이블, 인덱스, 뷰 등 사용자별로 여러 객체를 생성하므로 업무별 사용자를 생성한 후
		=> 각 사용자 업무에 맞는 데이터 구조를 만들어 관리하는 방식 사용
		
	- 데이터베이스 스키마
		
		* 스키마
			데이터베이스에서 데이터 간 관계, 데이터 구조, 제약 조건 등 데이터를 저장하고 관리하고자 정의한 데이터베이스 구조 범위
				=> 오라클DB 에서의 스키마 == 사용자
		
			
	- 사용자 생성 순서
		참고 : system(29)
		1-1) 기본 : CREATE USER 사용자이름 IDENTIFIED BY 비밀번호;
		
		1-2) CREATE USER 사용자이름 IDENTIFIED BY 비밀번호
		...
		2. 권한부여(GRANT)
			- 기본 : 개별로 권한 부여
				GRANT SELECT,INSERT,DELETE ON 테이블명 TO 유저명;
			...
			* ORA-65096: 공통 사용자 또는 롤 이름이 부적합합니다
				=> C## 없어서 그럼
				=> C## 넣기 싫을 때 : ALTER SESSION SET "_ORACLE_SCRIPT"=TRUE;
				
	- 비밀번호 변경
		참고 : system(48)
		ALTER USER 사용자명 IDENTIFIED BY 바꿀비밀번호;
			=> 비밀번호 변경시 커넥션 끊고 다시 연결하기
			=> 관리자(system)에서 가능한 것
...
<< 데이터베이스 설계 >>
	...
	- 단계
		...
		2) 개념적 설계
			1. 개체와 속성 추출
				=> 명사 찾기 => 찾은 명사 개체와 속성으로 분류
			2. 관계 추출
			3. E-R 다이어그램 설계
			=> 결과물 : 개념적 스키마(E-R 다이어그램(ERD) : 그림 같은거)
		3) 논리적 설계
			=> 결과물 : 논리적 스키마(릴레이션 스키마 : 테이블모양 같은거)
		4) 물리적 설계
			=> 결과물 : 물리적 스키마
		5) 구현
